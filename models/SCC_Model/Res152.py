...
class Res152(nn.Module):
    def __init__(self, pretrained=True):
        ...
        res = models.resnet152(pretrained=pretrained)
        ...
        self.de_pred1 = nn.Sequential(Conv2d(1024, 256, 3, same_padding=True, NL='relu'),
                                     Conv2d(256, 32, 3, same_padding=True, NL='relu'))
      

        self.de_pred2 = nn.Sequential(Conv2d(1024, 64, 3, same_padding=True, NL='relu'),
                                     Conv2d(64, 32, 3, same_padding=True, NL='relu'))

        self.de_pred3 = nn.Sequential(Conv2d(1024, 256, 5, same_padding=True, NL='relu'),
                                     Conv2d(256, 32, 5, same_padding=True, NL='relu'))

        self.de_pred4 = nn.Sequential(Conv2d(1024, 64, 5, same_padding=True, NL='relu'),
    ...
    def forward(self, x):
        ...
        x_pred1 = self.de_pred1(x)
        x_pred2 = self.de_pred2(x)
        x_pred3 = self.de_pred3(x)
        x_pred4 = self.de_pred4(x)
        x = (x_pred1 + x_pred2 + x_pred3+ x_pred4)/4
        x = self.output(x)
